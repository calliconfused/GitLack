...
unsigned long   TIMER_CHECKPOINT;
long            TIMER_RESOLUTION = 1000;

int             TS_POSX;
int             TS_POSY;

int             SELECTED_FAN;   // 0 = OFF, 1 = 25%, 2 = 50%, 3 = 75%, 4 = FULL, 5 = AUTO (not implemented yet)
int             SELECTED_LED;   // 0 = OFF, 1 = 25%, 2 = 50%, 3 = 75%, 4 = FULL, 5 = AUTO (not implemented yet)
int             SELECTED_FAN_LAST;
int             SELECTED_LED_LAST;

int             POS_CIRCLE_X[] = {72, 208};
int             POS_CIRCLE_Y[] = {302, 275, 248, 221, 194};
char            *DESC_OPTIONS[] = {"OFF", "25%", "50%", "75%", "FULL"};
...
void loop() {
  ...
  if ( p.z > MINPRESSURE && p.z < MAXPRESSURE && TIMER_CHECKPOINT <= millis() ) {
...
/*
    ...
    Serial.print("\tX = "); Serial.print(TS_POSX);
    Serial.print("\tY = "); Serial.println(TS_POSY);
*/
    TIMER_CHECKPOINT = millis() + TIMER_RESOLUTION;
...
void vChangeButtons(int SELECTED_OPTION, int SELECTED_OPTION_LAST, int SELECTED_TYPE) {
  ...
  if ( SELECTED_OPTION != SELECTED_OPTION_LAST ) {
    ...
    if ( SELECTED_TYPE == 0 ) {
//      Serial.print("Selected FAN option: "); Serial.println(SELECTED_OPTION);
      tft.fillCircle(POS_CIRCLE_X[SELECTED_TYPE], POS_CIRCLE_Y[SELECTED_FAN_LAST], 7, COLOR_BLACK);
      SELECTED_FAN_LAST = SELECTED_OPTION;
    } else 
    
    if ( SELECTED_TYPE == 1 ) {
//      Serial.print("Selected LED option: "); Serial.println(SELECTED_OPTION);
